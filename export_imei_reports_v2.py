...
v2 changes (fixing failures seen in production run):
  - Status/Creation Date are re-verified and re-applied on EVERY date
    ...
    resetting them mid-loop (Status -> Any, Creation Date -> Today),
    which produced "more than 100 documents found" and broke the export.
  - Element interactions only touch VISIBLE matches, and log a warning
    when duplicate elements are found instead of silently grabbing the
    wrong (stale) one.
  - "More than 100 documents found" is detected as its own condition,
    distinct from zero results, and triggers a retry instead of a
    doomed wait for a download that isn't coming.
  - Each date gets up to 2 retries with full state re-verification
    before being logged as failed.
  - Structured logging to console + file, screenshot-on-failure, and
    an end-of-run summary (success / no-orders / too-many / failed).
"""
...
logger = logging.getLogger("imei_export")
logger.setLevel(logging.DEBUG)
logger.handlers.clear()

file_handler = logging.FileHandler(LOG_DIR / f"imei_export_{RUN_ID}.log", encoding="utf-8")
...
def log(market, msg, level="info"):
    getattr(logger, level)(f"[{market}] {msg}")
...
def merge_exports(file_paths, output_path, interactive=True):
    ...
    logger.info(f"Merged {len(file_paths)} files -> {final_path} ({len(merged)} rows)")
    return final_path
    ...
    for r in all_results:
        ...
        if r["failed"]:
            for date_str, reason in r["failed"].items():
                logger.info(f"    FAILED {date_str}: {reason}")
    logger.info("=" * 60)
